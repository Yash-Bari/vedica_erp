...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto bg-white shadow-md rounded-lg p-6">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Expense 
            <span class="text-sm text-gray-500 ml-2">#{{ $expense->id }}</span>
        </h1>
                ...
                <div>
                    ...
                    <select id="type" name="type" 
                        ...
                        @foreach(['Material', 'Labor', 'Equipment', 'Transportation', 'Miscellaneous'] as $type)
                            <option value="{{ $type }}" 
                                {{ old('type', $expense->type) == $type ? 'selected' : '' }}>
                                {{ $type }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="category" name="category" 
                        ...
                        <option value="Direct" {{ old('category', $expense->category) == 'Direct' ? 'selected' : '' }}>Direct</option>
                        <option value="Indirect" {{ old('category', $expense->category) == 'Indirect' ? 'selected' : '' }}>Indirect</option>
                    </select>
                    @error('category')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                        ...
                        placeholder="Add any additional details about the expense">{{ old('description', $expense->description) }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Optional: Dynamic project requirement based on expense type
        const typeSelect = document.getElementById('type');
        const projectSelect = document.getElementById('project_id');

        typeSelect.addEventListener('change', function() {
            const selectedType = this.value;
            const projectRequiredTypes = ['Equipment', 'Maintenance', 'Repair'];
            
            if (projectRequiredTypes.includes(selectedType)) {
