...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto bg-white shadow-md rounded-lg p-6">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">
            Edit Machine 
            <span class="text-sm text-gray-500 ml-2">#{{ $machine->id }}</span>
        </h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="model_number" name="model_number" 
                        value="{{ old('model_number', $machine->model_number) }}"
                ...
                <div>
                    ...
                    <select id="type" name="type" 
                        ...
                        @foreach(App\Models\Machine::TYPES as $type)
                            <option value="{{ $type }}" 
                                {{ old('type', $machine->type) == $type ? 'selected' : '' }}>
                                {{ $type }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="status" name="status" 
                        ...
                        @foreach(App\Models\Machine::STATUS as $status)
                            <option value="{{ $status }}" 
                                {{ old('status', $machine->status) == $status ? 'selected' : '' }}>
                                {{ $status }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select id="project_id" name="project_id" 
                        ...
                        @foreach($projects as $project)
                            <option value="{{ $project->id }}" 
                                {{ old('project_id', $machine->project_id) == $project->id ? 'selected' : '' }}>
                                {{ $project->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" id="operating_weight" name="operating_weight" step="0.01" min="0"
                        value="{{ old('operating_weight', $machine->operating_weight) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" id="fuel_capacity" name="fuel_capacity" step="0.01" min="0"
                        value="{{ old('fuel_capacity', $machine->fuel_capacity) }}"
                ...
                <div>
                    ...
                    <input type="text" id="current_location" name="current_location" 
                        value="{{ old('current_location', $machine->current_location) }}"
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="3"
                        ...
                        placeholder="Any additional information about the machine">{{ old('notes', $machine->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
